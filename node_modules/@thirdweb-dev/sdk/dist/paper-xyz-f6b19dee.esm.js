import { z } from 'zod';
import { N as NFTInputOrUriSchema, C as CommonNFTOutput, a as CommonNFTInput, _ as _defineProperty } from './defineProperty-6292d8c3.esm.js';
import { BigNumber, utils, constants, providers, ethers, Signer, Contract } from 'ethers';
...
const FileOrBufferUnionSchema = isBrowser$1() ? z.instanceof(File) : z.instanceof(Buffer); // @fixme, this is a hack to make browser happy for now
  ...
  constructor(reason, from, to, data, network, rpcUrl, raw, functionInfo) {
    ...
    if (functionInfo) {
      ...
      if (functionInfo.value.gt(0)) {
        ...
        builtErrorMsg += withSpaces("value", `${ethers.utils.formatEther(functionInfo.value)} ${(_NATIVE_TOKENS = NATIVE_TOKENS[network.chainId]) === null || _NATIVE_TOKENS === void 0 ? void 0 : _NATIVE_TOKENS.symbol}`);
      }
    }
...
async function createSnapshot(snapshotInput, tokenDecimals, storage) {
  ...
  const hashedLeafs = input.map(i => hashLeafNode(i.address, utils.parseUnits(i.maxClaimable, tokenDecimals))); // import dynamically to avoid bloating bundle size for non-merkle tree users
...
async function extractIPFSHashFromBytecode(bytecode) {
  ...
  const [cbor, multiHashes] = await Promise.all([import('cbor-web'), import('multihashes')]);
...
class Erc20Claimable {
  ...
  constructor(erc20, contractWrapper, storage) {
    _defineProperty(this, "featureName", FEATURE_TOKEN_CLAIMABLE_WITH_CONDITIONS.name);
...
class Erc721ClaimableWithConditions {
  ...
  constructor(erc721, contractWrapper, storage) {
    _defineProperty(this, "featureName", FEATURE_NFT_CLAIMABLE_WITH_CONDITIONS.name);
...
class ERC1155Claimable {
  constructor(contractWrapper) {
    _defineProperty(this, "featureName", FEATURE_EDITION_CLAIMABLE.name);
...
class Erc1155ClaimableWithConditions {
  constructor(contractWrapper, storage) {
    _defineProperty(this, "featureName", FEATURE_EDITION_CLAIMABLE_WITH_CONDITIONS.name);
...
class Erc1155LazyMintable {
  ...
  constructor(erc1155, contractWrapper, storage) {
    _defineProperty(this, "featureName", FEATURE_EDITION_LAZY_MINTABLE.name);
...
const EditionDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([EditionDropInitializer.getAbi(), import('./edition-drop-8decc040.esm.js')]);
    return new contract.EditionDrop(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/DropERC1155.json')).default
};
const EditionInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([EditionInitializer.getAbi(), import('./edition-2f24c13c.esm.js')]);
    return new contract.Edition(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/TokenERC1155.json')).default
};
const MarketplaceInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([MarketplaceInitializer.getAbi(), import('./marketplace-1d25acb3.esm.js')]);
    return new contract.Marketplace(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/Marketplace.json')).default
};
const MultiwrapInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([MultiwrapInitializer.getAbi(), import('./multiwrap-fa462eaf.esm.js')]);
    return new contract.Multiwrap(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/Multiwrap.json')).default
};
const NFTCollectionInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([NFTCollectionInitializer.getAbi(), import('./nft-collection-c2cbafb5.esm.js')]);
    return new contract.NFTCollection(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/TokenERC721.json')).default
};
const NFTDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([NFTDropInitializer.getAbi(), import('./nft-drop-98ca694c.esm.js')]);
    return new contract.NFTDrop(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/DropERC721.json')).default
};
const PackInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([PackInitializer.getAbi(), import('./pack-e44dd56f.esm.js')]);
    return new contract.Pack(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/Pack.json')).default
};
const SignatureDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([SignatureDropInitializer.getAbi(), import('./signature-drop-8acbe551.esm.js')]);
    return new contract.SignatureDrop(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/SignatureDrop.json')).default
};
const SplitInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([SplitInitializer.getAbi(), import('./split-e6a60a09.esm.js')]);
    return new contract.Split(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/Split.json')).default
};
const TokenDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([TokenDropInitializer.getAbi(), import('./token-drop-1a2b14f6.esm.js')]);
    return new contract.TokenDrop(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/DropERC20.json')).default
};
const TokenInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([TokenInitializer.getAbi(), import('./token-05c71c98.esm.js')]);
    return new contract.Token(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/TokenERC20.json')).default
};
const VoteInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract] = await Promise.all([VoteInitializer.getAbi(), import('./vote-036364cf.esm.js')]);
    return new contract.Vote(network, address, storage, options, abi);
  },
  getAbi: async () => (await import('@thirdweb-dev/contracts-js/dist/abis/VoteERC20.json')).default
};
/**
...
class ThirdwebSDK extends RPCConnectionHandler {
  ...
  async resolveContractType(contractAddress) {
    try {
      const contract = new Contract(contractAddress, IThirdwebContractABI, // !provider only! - signer can break things here!
      this.getProvider());
...
export { ExtraPublishMetadataSchemaOutput as $, MintRequest721withQuantity as A, BaseSignaturePayloadInput as B, CommonContractSchema as C, MerkleSchema as D, EditionMetadataOutputSchema as E, SnapshotAddressInput as F, SnapshotInputSchema as G, SnapshotSchema as H, SnapshotInfoSchema as I, BYOCContractMetadataSchema as J, CustomContractInput as K, CustomContractOutput as L, MintRequest20 as M, CustomContractDeploy as N, OptionalPropertiesInput as O, PartialClaimConditionInputSchema as P, QuantitySchema as Q, CustomContractSchema as R, SUPPORTED_CHAIN_IDS as S, TokenMintInputSchema as T, AbiTypeSchema as U, AbiObjectSchema as V, AbiSchema as W, PreDeployMetadata as X, ChainIdToAddressSchema as Y, FactoryDeploymentSchema as Z, ExtraPublishMetadataSchemaInput as _, ClaimConditionInputSchema as a, ListingType as a$, FullPublishMetadataSchemaInput as a0, FullPublishMetadataSchemaOutput as a1, ProfileSchemaInput as a2, ProfileSchemaOutput as a3, PublishedContractSchema as a4, ContractInfoSchema as a5, CompilerMetadataFetchedSchema as a6, PreDeployMetadataFetchedSchema as a7, LoginOptionsSchema as a8, LoginPayloadDataSchema as a9, Erc721Supply as aA, Erc721Enumerable as aB, Erc721Mintable as aC, Erc721BatchMintable as aD, Erc721Burnable as aE, StandardErc721 as aF, Erc1155 as aG, Erc1155Enumerable as aH, Erc1155Mintable as aI, Erc1155BatchMintable as aJ, Erc1155SignatureMintable as aK, Erc1155Burnable as aL, Erc1155LazyMintable as aM, StandardErc1155 as aN, MarketplaceDirect as aO, MarketplaceAuction as aP, ContractDeployer as aQ, GasCostEstimator as aR, ContractEvents as aS, ContractInterceptor as aT, ContractPlatformFee as aU, ContractPublishedMetadata as aV, ContractOwner as aW, TransactionTask as aX, UserWallet as aY, ThirdwebSDK as aZ, DEFAULT_QUERY_ALL_COUNT as a_, LoginPayloadSchema as aa, VerifyOptionsSchema as ab, AuthenticationOptionsSchema as ac, AuthenticationPayloadDataSchema as ad, AuthenticationPayloadSchema as ae, WalletAuthenticator as af, ContractEncoder as ag, ContractMetadata as ah, ContractRoles as ai, ContractRoyalty as aj, ContractPrimarySale as ak, DelayedReveal as al, DropClaimConditions as am, DropErc1155ClaimConditions as an, DropErc1155History as ao, Erc20 as ap, Erc20Mintable as aq, Erc20BatchMintable as ar, TokenERC20History as as, Erc20SignatureMintable as at, Erc20Burnable as au, StandardErc20 as av, Erc721 as aw, Erc721LazyMintable as ax, Erc721ClaimableWithConditions as ay, Erc721WithQuantitySignatureMintable as az, ClaimConditionInputArray as b, TW_IPFS_SERVER_URL as b$, ProposalState as b0, VoteType as b1, ClaimEligibility as b2, NotFoundError as b3, InvalidAddressError as b4, MissingRoleError as b5, AssetNotFoundError as b6, UploadError as b7, FileNameMissingError as b8, DuplicateFileNameError as b9, resolveContractUriFromAddress as bA, fetchContractMetadataFromAddress as bB, fetchContractMetadata as bC, fetchSourceFilesFromMetadata as bD, fetchRawPredeployMetadata as bE, fetchPreDeployMetadata as bF, fetchExtendedReleaseMetadata as bG, detectFeatures as bH, isFeatureEnabled as bI, assertEnabled as bJ, detectContractFeature as bK, hasFunction as bL, toSemver as bM, isIncrementalVersion as bN, isDowngradeVersion as bO, OZ_DEFENDER_FORWARDER_ADDRESS as bP, CONTRACT_ADDRESSES as bQ, getContractAddressByChainId as bR, getContractPublisherAddress as bS, InterfaceId_IERC721 as bT, InterfaceId_IERC1155 as bU, NATIVE_TOKEN_ADDRESS as bV, NATIVE_TOKENS as bW, getNativeTokenByChainId as bX, EventType as bY, DEFAULT_IPFS_GATEWAY as bZ, PUBLIC_GATEWAYS as b_, NotEnoughTokensError as ba, MissingOwnerRoleError as bb, QuantityAboveLimitError as bc, FetchError as bd, DuplicateLeafsError as be, AuctionAlreadyStartedError as bf, FunctionDeprecatedError as bg, ListingNotFoundError as bh, WrongListingTypeError as bi, RestrictedTransferError as bj, AdminRoleMissingError as bk, AuctionHasNotEndedError as bl, ExtensionNotImplementedError as bm, TransactionError as bn, convertToTWError as bo, includesErrorMessage as bp, createSnapshot as bq, hashLeafNode as br, ALL_ROLES as bs, getRoleHash as bt, extractConstructorParams as bu, extractFunctions as bv, extractConstructorParamsFromAbi as bw, extractFunctionParamsFromAbi as bx, extractFunctionsFromAbi as by, extractEventsFromAbi as bz, ClaimConditionOutputSchema as c, PINATA_IPFS_URL as c0, getProviderForNetwork as c1, getReadOnlyProvider as c2, EditionDropInitializer as c3, EditionInitializer as c4, MarketplaceInitializer as c5, MultiwrapInitializer as c6, NFTCollectionInitializer as c7, NFTDropInitializer as c8, PackInitializer as c9, FEATURE_NFT_REVEALABLE as cA, uploadOrExtractURIs as cB, prepareClaim as cC, AddressSchema as cD, AmountSchema as cE, BigNumberishSchema as cF, RawDateSchema as cG, PackContractSchema as cH, SplitsContractSchema as cI, fetchCurrencyValue as cJ, DropErc20ContractSchema as cK, TokenErc20ContractSchema as cL, VoteContractSchema as cM, SignatureDropInitializer as ca, SplitInitializer as cb, TokenDropInitializer as cc, TokenInitializer as cd, VoteInitializer as ce, PREBUILT_CONTRACTS_MAP as cf, CONTRACTS_MAP as cg, getContractTypeForRemoteName as ch, PAPER_API_URL as ci, parseChainIdToPaperChain as cj, fetchRegisteredCheckoutId as ck, createCheckoutLinkIntent as cl, PaperCheckout as cm, ContractWrapper as cn, DropErc1155ContractSchema as co, TokenErc1155ContractSchema as cp, MarketplaceContractSchema as cq, mapOffer as cr, MultiwrapContractSchema as cs, fetchCurrencyMetadata as ct, uploadOrExtractURI as cu, normalizePriceValue as cv, hasERC20Allowance as cw, isTokenApprovedForTransfer as cx, TokenErc721ContractSchema as cy, DropErc721ContractSchema as cz, ChainId as d, EditionMetadataWithOwnerOutputSchema as e, EditionMetadataInputSchema as f, EditionMetadataInputOrUriSchema as g, CommonContractOutputSchema as h, CommonRoyaltySchema as i, CommonPrimarySaleSchema as j, CommonPlatformFeeSchema as k, CommonTrustedForwarderSchema as l, CommonSymbolSchema as m, CurrencySchema as n, CurrencyValueSchema as o, Signature20PayloadInput as p, Signature20PayloadOutput as q, Signature721PayloadInput as r, Signature721PayloadOutput as s, Signature1155PayloadInput as t, Signature1155PayloadInputWithTokenId as u, Signature1155PayloadOutput as v, Signature721WithQuantityInput as w, Signature721WithQuantityOutput as x, MintRequest721 as y, MintRequest1155 as z };
